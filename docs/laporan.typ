...
This study examined how Ant Colony Optimization scales on the Capacitated Vehicle Routing Problem as the number of customers grows from 25 to 100 using the Solomon RC101 benchmark. Among the four configurations tested, C3 — characterised by a higher pheromone evaporation rate — consistently produced the shortest routes at the largest instance size and exhibited the most stable convergence behavior across scales. However, the Clarke-Wright Savings heuristic outperformed all ACO configurations at 100 customers, underscoring that stochastic search incurs an overhead that deterministic construction avoids at moderate problem scales. These results suggest that when deploying ACO for large-scale routing, an evaporation rate of at least 0.3 should be preferred, and configurations that combine strong heuristic bias with slow evaporation should be avoided due to the risk of premature convergence. Future work should replicate this study with a larger number of independent runs and a wider range of benchmark instances to strengthen statistical conclusions and generalise the scalability findings.